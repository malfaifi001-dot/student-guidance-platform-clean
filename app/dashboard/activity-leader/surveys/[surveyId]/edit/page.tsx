...
export default async function ActivityLeaderSurveyEditPage({ params }: PageProps) {
  ...
  if (current.user.role !== "ACTIVITY_LEADER") {
    redirect(`/dashboard/surveys/${surveyId}/edit`);
  }

  return <SurveyEditShell surveyId={surveyId} boardPath="/dashboard/activity-leader/surveys" />;
}