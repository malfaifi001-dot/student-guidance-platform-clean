...
export default async function ActivityLeaderSurveyResponsesPage({ params }: PageProps) {
  ...
  if (current.user.role !== "ACTIVITY_LEADER") {
    redirect(`/dashboard/surveys/${surveyId}/responses`);
  }

  return <SurveyResponsesShell surveyId={surveyId} boardPath="/dashboard/activity-leader/surveys" />;
}