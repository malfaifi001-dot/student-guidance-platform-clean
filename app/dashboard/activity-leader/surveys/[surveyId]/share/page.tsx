...
export default async function ActivityLeaderSurveySharePage({ params }: PageProps) {
  ...
  if (current.user.role !== "ACTIVITY_LEADER") {
    redirect(`/dashboard/surveys/${surveyId}/share`);
  }

  return <SurveyShareShell surveyId={surveyId} boardPath="/dashboard/activity-leader/surveys" />;
}