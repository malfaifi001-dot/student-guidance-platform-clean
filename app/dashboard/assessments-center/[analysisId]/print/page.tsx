...
export default async function AssessmentPrintPage({ params, searchParams }: { params: Promise<{ analysisId: string }>; searchParams: Promise<{ print?: string | string[] }> }) {
  ...
  const analysis = await prisma.assessmentAnalysis.findFirst({ where: { id: routeParams.analysisId, ...(context.isAdmin ? {} : assessmentAnalysisOwnershipWhere(context.schoolAccountId, context.user.id, { historicalPersonalRead: true })), uploadMode: { in: ["NAFS", "NAFS_PRE_POST", "MAHIROON", "SUBJECT_PERIODIC", "LEARNING_STYLE"] } }, select: { summaryJson: true, uploadMode: true } });
