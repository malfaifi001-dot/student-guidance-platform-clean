...
export default async function NafsDevelopmentPlanPage({ params }: { params: Promise<{ analysisId: string }> }) {
  ...
  const analysis = await prisma.assessmentAnalysis.findFirst({ where: { id: (await params).analysisId, ...(context.isAdmin ? {} : { schoolAccountId: context.schoolAccountId }), uploadMode: "NAFS" }, select: { summaryJson: true } });
