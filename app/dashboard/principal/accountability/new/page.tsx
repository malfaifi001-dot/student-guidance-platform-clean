...
export default async function NewPrincipalAccountabilityPage() {
  ...
  return <main dir="rtl"><AccountabilityCreateForm workflow={workflow.workflow} serviceId={access.service.id} schoolName={access.user.schoolAccount?.profile?.schoolName || access.user.schoolAccount?.name} members={members.map((member) => ({ id: member.id, name: member.officialName || member.name, role: member.role }))} /></main>;
}
