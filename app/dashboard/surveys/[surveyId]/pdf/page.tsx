...
export default async function CounselorSurveyPdfPage({ params }: PageProps) {
  ...
  if (current.user.role === "ACTIVITY_LEADER") {
    redirect(`/dashboard/activity-leader/surveys/${surveyId}/pdf`);
  }

  return (
    <SurveyPrintReportShell
      surveyId={surveyId}
      backPath={`/dashboard/surveys/${surveyId}/analysis`}
    />
  );
}