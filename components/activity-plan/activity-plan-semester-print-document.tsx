...
function groupRows(rows: ActivityPlanTenPercentRow[]) {
  const grouped = new Map<string, ActivityPlanTenPercentRow[]>();
  for (const row of rows) {
    const combinations = row.grades.length ? row.grades : [""];
    for (const combination of combinations) {
      const key = combination || `${row.id}:unassigned`;
      grouped.set(key, [...(grouped.get(key) || []), { ...row, grades: combination ? [combination] : [] }]);
    }
  }
  return Array.from(grouped.entries()).flatMap(([key, pageRows]) => {
    const pages: Array<[string, ActivityPlanTenPercentRow[]]> = [];
    for (let index = 0; index < pageRows.length; index += 8) pages.push([`${key}:${index}`, pageRows.slice(index, index + 8)]);
    return pages;
  });
}
...
export function ActivityPlanSemesterPrintDocument({ rows, gradeSections = [], stage, academicYear, schoolName, educationDepartment, logoUrl, activityLeaderName, activityLeaderSignatureUrl, principalName, principalSignatureUrl }: Props) {
  ...
  const pages = groupRows(scopedRows);
