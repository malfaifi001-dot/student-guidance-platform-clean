...
export function WeeklyActivityPlanPanel({ stage }: { stage: string }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.error || "تعذر تحميل الخطة الأسبوعية.");
      setWeeks(payload.weeks || []);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "تعذر تحميل الخطة الأسبوعية."); }
  ...
  return <>
    ...
    {loading ? <p className="py-4 text-center text-xs font-black text-slate-400">جار تحميل الخطة الأسبوعية...</p> : null}
