...
export function WeeklyActivityPlanPrintDocument({ weeks, stage, academicYear, schoolName, educationDepartment, logoUrl, activityLeaderName, activityLeaderSignatureUrl, principalName, principalSignatureUrl }: Props) {
  const items: ActivityPlanPhysicalItem[] = groups(weeks).map((group, index) => ({ id: `band-${index}`, node: <div data-activity-plan-row className="weekly-plan-band"><Band weeks={group} /></div> }));
  return <>
    <style>{`.weekly-plan-a4{width:100%;display:flex;flex-direction:column}.weekly-plan-band,.weekly-plan-band-frame{break-inside:avoid-page;page-break-inside:avoid}.weekly-plan-bands{display:flex;flex-direction:column;gap:8px}`}</style>
    <style>{activityPlanPrintShellStyles}</style><main className="activity-plan-print-root" dir="rtl"><ActivityPlanPhysicalPaginator items={items.length ? items : [{ id: "empty", node: <div data-activity-plan-row /> }]} renderPage={({ items: pageItems, includeSignatures }) => <ActivityPlanPrintPage className={`weekly-activity-plan-print-page activity-plan-print-page--physical${includeSignatures ? "" : " activity-plan-print-page--compact-footer"}`} footer={<CurriculumDocumentFooter primaryRoleLabel="رائد النشاط" primaryName={activityLeaderName} primarySignatureUrl={activityLeaderSignatureUrl} primarySignatureAlt="توقيع رائد النشاط" principalName={principalName} principalSignatureUrl={principalSignatureUrl} includeSignatures={includeSignatures} />}>
        <div className="weekly-plan-a4">
          ...
          <div className="weekly-plan-bands">{pageItems.map((item) => <Fragment key={item.id}>{item.node}</Fragment>)}</div>
        </div>
      </ActivityPlanPrintPage>} /></main>
  </>;
}
