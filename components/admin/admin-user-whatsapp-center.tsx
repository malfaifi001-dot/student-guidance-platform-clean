...
import { buildMembershipInvitationMessage, buildWhatsAppLink, normalizeSaudiWhatsAppNumber } from "@/lib/whatsapp/whatsapp-links";
  ...
  return (
    <main className="space-y-6 pb-24" dir="rtl">
      ...
      <section className="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm">
        {loading ? <div className="grid min-h-64 place-items-center text-sm font-black text-slate-500"><span className="flex items-center gap-2"><Loader2 className="h-5 w-5 animate-spin text-sky-600" />جارٍ تحميل المستخدمين...</span></div> : data?.users.length ? <div className="divide-y divide-slate-100">{data.users.map((user) => <WhatsAppUserRow key={user.id} user={user} />)}</div> : <div className="px-6 py-16 text-center text-sm font-bold text-slate-500">{search.trim() ? "لا توجد نتائج مطابقة" : "لا يوجد مستخدمون لعرضهم حاليًا"}</div>}
      </section>
...
function WhatsAppUserRow({ user }: { user: WhatsAppUser }) {
  const normalized = normalizeSaudiWhatsAppNumber(user.phone);
  const chatLink = buildWhatsAppLink(user.phone);
  const messageLink = buildWhatsAppLink(user.phone, buildMembershipInvitationMessage(user.name));
