...
export function AssessmentNewClient() {
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [grade, classroom, studentLoadVersion, periods]);
  ...
  async function save() {
    ...
    try {
      response = await fetch("/api/dashboard/assessments-center", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ type, title, subject, grade, classroom, maximumScore: max, inputMode: mode, periods, students: normalized }),
      });
    } catch {
  ...
  return <main dir="rtl" className="mx-auto max-w-7xl space-y-6 p-6">
    <header className="rounded-3xl bg-white p-6 shadow-sm"><p className="font-bold text-blue-700">مركز التحاليل والاختبارات</p><h1 className="mt-2 text-3xl font-black">تحليل جديد</h1></header>
