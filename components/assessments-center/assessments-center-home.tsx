...
export function AssessmentsCenterHome() {
  ...
  return <main dir="rtl" className="mx-auto max-w-7xl space-y-6 p-6">
    ...
    <section className="rounded-3xl border bg-white p-6 shadow-sm">
      ...
      {loading ? <p className="flex items-center justify-center gap-2 py-12 font-bold text-slate-500"><Loader2 className="h-5 w-5 animate-spin" /> جارٍ التحميل...</p> : analyses.length === 0 ? <div className="py-16 text-center"><p className="text-xl font-black">لا توجد تحليلات محفوظة</p><p className="mt-2 font-bold text-slate-500">ابدأ بإنشاء تحليل جديد لنتائج الطلاب.</p><Link href="/dashboard/assessments-center/new" className="mt-5 inline-flex rounded-xl bg-teal-700 px-5 py-3 font-black text-white">تحليل جديد</Link></div> : <div className="mt-5 grid gap-4 md:grid-cols-2">{analyses.map((analysis) => { const existingLink = linkByAnalysis.get(analysis.id) || null; return <article key={analysis.id} className="rounded-2xl border border-slate-200 p-5"><div className="flex items-start justify-between gap-3"><div><span className="rounded-full bg-cyan-50 px-3 py-1 text-xs font-black text-cyan-700">{labels[analysis.uploadMode] || analysis.uploadMode}</span><h3 className="mt-3 text-lg font-black">{analysis.title}</h3></div><span className="text-xs font-bold text-slate-400">{new Date(analysis.updatedAt).toLocaleDateString("ar-SA")}</span></div><p className="mt-3 text-sm font-bold text-slate-500">عدد الطلاب: {analysis.totalStudents} · آخر متوسط: {analysis.averagePercentage ?? "-"}%</p><div className="mt-4 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap"><Link href={`/dashboard/assessments-center/${analysis.id}/print`} className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-100 px-3 py-2 text-sm font-black text-slate-700 transition hover:bg-slate-200"><Eye className="h-4 w-4" />معاينة</Link><Link href={`/dashboard/assessments-center/${analysis.id}?mode=edit`} className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-50 px-3 py-2 text-sm font-black text-amber-800"><Edit3 className="h-4 w-4" />تعديل</Link><button type="button" onClick={() => void print.runPrintExport({ exportUrl: `/api/dashboard/assessments-center/${analysis.id}/export/pdf`, printUrl: `/dashboard/assessments-center/${analysis.id}/print?print=1`, fileName: `${analysis.title || "تحليل نتائج"}.pdf`, blockedTitle: "معاينة التقرير" })} className="inline-flex items-center justify-center gap-2 rounded-xl bg-teal-50 px-3 py-2 text-sm font-black text-teal-700 disabled:opacity-60" disabled={print.status === "loading"}><Printer className="h-4 w-4" />تحميل</button><button type="button" onClick={() => setLinkAnalysis(analysis)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-sky-50 px-3 py-2 text-sm font-black text-sky-700"><Link2 className="h-4 w-4" />{existingLink ? "تعديل الربط" : "ربط"}</button><button type="button" onClick={() => { setDeleteError(""); setDeleteAnalysis(analysis); }} className="inline-flex items-center justify-center gap-2 rounded-xl bg-rose-50 px-3 py-2 text-sm font-black text-rose-700 transition hover:bg-rose-100"><Trash2 className="h-4 w-4" />حذف</button></div>{existingLink ? <p className="mt-3 text-xs font-black text-emerald-700">مرتبط بملف الإنجاز</p> : null}</article>; })}</div>}
    </section>
