...
function ReportBrand({ data }: { data: AssessmentAnalyticalReportData }) {
  return <div className="report-brand">
    <div className="report-brand__identity">{data.school.ministryLogoUrl ? <img src={data.school.ministryLogoUrl} alt="" className="report-brand__ministry" /> : <div className="report-brand__ministry-placeholder">وزارة التعليم</div>}</div>
    <div className="report-brand__school"><strong>{data.school.name || "مدرسة Teachix"}</strong>{data.school.educationAdministration ? <span>{data.school.educationAdministration}</span> : null}{data.school.educationOffice ? <span>{data.school.educationOffice}</span> : null}</div>
    <div className="report-brand__school-logo">{data.school.logoUrl ? <img src={data.school.logoUrl} alt="" /> : <div className="report-brand__school-mark">T</div>}</div>
  </div>;
}
...
function MetricCard({ label, value, hint, tone = "default" }: { label: string; value: React.ReactNode; hint?: string; tone?: "default" | "green" | "blue" | "amber" | "red" }) {
  return <div className={`metric-card metric-card--${tone}`}><div className="metric-card__label">{label}</div><div className="metric-card__value">{value}</div>{hint ? <div className="metric-card__hint">{hint}</div> : null}</div>;
}
...
function AnalyticalReportPages({
  ...
  const flowBlocks = useMemo<AssessmentFlowBlock[]>(() => {
    ...
    if (data.analysis.followUpIndicators?.length) {
      blocks.push({
        ...
        node: (
          <div className="follow-up-box">
            ...
            <InsightList items={data.analysis.followUpIndicators} />
          </div>
        ),
      });
    }
...
export function AssessmentAnalyticalReport({ data }: Props) {
  ...
  return <div className="assessment-report" dir="rtl">
    ...
    <style jsx global>{`
      ...
      .report-brand{display:grid;grid-template-columns:145px minmax(0,1fr) 58px;gap:14px;align-items:center;padding-bottom:14px;border-bottom:2px solid #e2ebf2;margin-bottom:20px}
      .report-brand__identity{display:flex;justify-content:flex-start;align-items:center}.report-brand__ministry{max-height:58px;max-width:145px;object-fit:contain}.report-brand__ministry-placeholder{font-size:19px;font-weight:800;color:var(--report-teal)}.report-brand__school{display:flex;flex-direction:column;text-align:center;gap:2px;line-height:1.6}.report-brand__school strong{color:var(--report-navy);font-size:19px;font-weight:900}.report-brand__school span{font-size:13px;color:#536b80;font-weight:700}.report-brand__school-logo{display:flex;justify-content:flex-end}.report-brand__school-logo img{width:56px;height:56px;object-fit:contain}.report-brand__school-mark{width:50px;height:50px;border-radius:16px;background:linear-gradient(145deg,var(--report-blue),var(--report-teal));color:#fff;display:grid;place-items:center;font-weight:900;font-size:21px}
