    ...
    <div dir="rtl" className="curriculum-distribution-shell space-y-5">
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 md:p-5">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {fields.map((field) => <label key={field.key} className="min-w-0 space-y-2"><span className="block text-xs font-black text-slate-600 dark:text-slate-300">{field.label}</span><span className="relative block"><select disabled={loading} value={field.value?.id || ""} onChange={(event) => { const value = field.choices.find((item) => item.id === event.target.value) || null; void field.onChange(value); }} className="min-h-11 w-full appearance-none rounded-xl border border-slate-200 bg-slate-50 px-4 pe-10 text-sm font-black text-slate-800 outline-none transition hover:border-sky-300 focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-100 disabled:cursor-wait disabled:opacity-60 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:focus:bg-slate-700"><option value="">اختر {field.label}</option>{field.choices.map((item) => <option key={item.id} value={item.id}>{item.name}{item.isExtra ? " (إضافية)" : ""}</option>)}</select><ChevronDown className="pointer-events-none absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" aria-hidden="true" /></span></label>)}
        </div>
        {error ? <p className="mt-3 rounded-xl border border-red-100 bg-red-50 p-2.5 text-sm font-bold text-red-700 dark:border-red-900/60 dark:bg-red-950/30 dark:text-red-200">{error}</p> : null}
        <div className="mt-4 flex flex-col items-start justify-between gap-2 border-t border-slate-100 pt-3 dark:border-slate-800 sm:flex-row sm:items-center">
          <p className="text-sm font-bold text-slate-500">{subject ? "أصبحت الخطة جاهزة للعرض." : "أكمل الاختيارات لعرض التوزيع."}</p>
          <div className="flex w-full flex-wrap gap-2 sm:w-auto sm:justify-end">
             {!distribution ? <button type="button" disabled={!subject || loading} onClick={showDistribution} className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-xl bg-slate-900 px-3.5 py-2 text-xs font-black text-white shadow-sm transition hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300 disabled:cursor-not-allowed disabled:opacity-45">
            ...
            {distribution ? <button type="button" disabled={print.status === "loading"} onClick={openDistributionPreview} className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs font-black text-slate-700 transition hover:border-sky-200 hover:bg-sky-50 hover:text-sky-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-200 disabled:cursor-wait disabled:opacity-60"><Eye className="h-4 w-4" />معاينة وتحميل</button> : null}

          </div>
        </div>
      </section>
       {distribution ? <div className="rounded-xl border border-sky-100 bg-sky-50/60 px-3 py-2.5 text-xs font-bold text-sky-800 dark:border-sky-900/60 dark:bg-sky-950/20 dark:text-sky-200">تم تحميل التوزيع. استخدم «معاينة وتحميل» للاطلاع الكامل.</div> : null}
       {!publicPreview ? <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 md:p-5" aria-labelledby="my-curriculum-title">
         <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-2.5 dark:border-slate-800">
           <div><h2 id="my-curriculum-title" className="text-base font-black text-slate-950 dark:text-slate-100">منهجي</h2><p className="mt-1 text-xs font-bold text-slate-500 dark:text-slate-400">المواد التي حفظتها للمتابعة.</p></div>
           {myCurriculumLoading ? <Loader2 className="h-4 w-4 animate-spin text-sky-600" aria-label="جار تحميل منهجي" /> : null}
         </div>
         {myCurriculum.length ? <div className="mt-3 grid gap-2.5 lg:grid-cols-2">{myCurriculum.map((item) => <MyCurriculumCard key={item.id} item={item} onRefresh={() => void loadMyCurriculum()} onSend={setSendItem} onPreview={(saved) => void runDistributionPreview({ subjectId: saved.subjectId, semesterId: saved.semesterId, stageId: saved.stage.id, gradeId: saved.grade.id })} />)}</div> : <p className="mt-3 rounded-xl border border-dashed border-slate-200 bg-slate-50/70 px-3 py-4 text-center text-sm font-black text-slate-500 dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-400">لا توجد مواد محفوظة في منهجي حتى الآن.</p>}
      </section> : null}
...
export function DistributionView({ distribution }: { distribution: CurriculumDistribution }) {
  ...
  return <section className="space-y-3" dir="rtl"><div className="rounded-xl bg-gradient-to-br from-sky-900 via-sky-800 to-cyan-700 p-3 text-white shadow-md shadow-sky-900/10 md:p-4"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">{summary.map(([label, value]) => <div key={label} className="min-w-0 rounded-lg bg-white/10 p-2 ring-1 ring-white/10"><span className="block text-[10px] font-bold text-white/65">{label}</span><strong className="mt-0.5 block truncate text-xs font-black text-white" title={value}>{value}</strong></div>)}</div></div><div className="grid gap-3 md:grid-cols-2">{calendarItems.map((item) => <CurriculumWeekCard key={item.id} item={item} />)}</div></section>;
}
