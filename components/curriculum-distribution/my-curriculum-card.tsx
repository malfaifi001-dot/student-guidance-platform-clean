...
export function MyCurriculumCard({ item, onRefresh, onSend, onPreview }: { item: SavedCurriculumItem; onRefresh: () => void; onSend: (item: SavedCurriculumItem) => void; onPreview: (item: SavedCurriculumItem) => void }) {
  ...
  return <>
    <article className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm transition hover:border-sky-200 hover:shadow-md dark:border-slate-700 dark:bg-slate-900 dark:hover:border-sky-700">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h3 className="truncate text-base font-black text-slate-950 dark:text-slate-100" title={item.subject.name}>{item.subject.name}</h3>
          <div className="mt-1.5 flex flex-wrap gap-1.5 text-[11px] font-bold text-slate-500 dark:text-slate-400"><span className="rounded-full bg-slate-50 px-2.5 py-1 dark:bg-slate-800">{item.stage.name}</span><span className="rounded-full bg-slate-50 px-2.5 py-1 dark:bg-slate-800">{item.grade.name}</span><span className="rounded-full bg-slate-50 px-2.5 py-1 dark:bg-slate-800">{item.semester.name}</span></div>
          {item.portfolioLink ? <p className="mt-1.5 text-xs font-black text-sky-700 dark:text-sky-300">{COPY.linked}</p> : null}
        </div>
        <ExpandableActionMenu menuId={`my-curriculum-${item.id}`} overlayStrip>
          <button type="button" onClick={() => onPreview(item)} aria-label={COPY.preview} title={COPY.preview} className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:border-sky-200 hover:bg-sky-50 hover:text-sky-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"><Eye className="h-4 w-4" /></button>
          <button type="button" onClick={() => setLinkOpen(true)} aria-label={item.portfolioLink ? COPY.editLink : COPY.link} title={item.portfolioLink ? COPY.editLink : COPY.link} className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:border-sky-200 hover:bg-sky-50 hover:text-sky-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"><Link2 className="h-4 w-4" /></button>
          <button type="button" onClick={() => onSend(item)} aria-label={COPY.send} title={COPY.send} className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:border-sky-200 hover:bg-sky-50 hover:text-sky-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"><Send className="h-4 w-4" /></button>
