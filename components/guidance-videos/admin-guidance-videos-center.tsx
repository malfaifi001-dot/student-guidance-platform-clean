      ...
      {formOpen ? (
        <form onSubmit={saveVideo} className="rounded-[2rem] border border-sky-100 bg-white p-5 shadow-sm sm:p-6 dark:border-sky-500/20 dark:bg-slate-950">
          <div className="flex items-center justify-between gap-3">
            <div><h2 className="text-xl font-black text-slate-950 dark:text-white">{editingId ? "تعديل الفيديو الإرشادي" : "إضافة فيديو إرشادي"}</h2><p className="mt-1 text-xs font-bold text-slate-400">{editingId ? "اترك ملف الفيديو فارغًا للاحتفاظ بالملف الحالي." : "ملفات MP4 فقط، بحد أقصى 100 ميجابايت."}</p></div>
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-2">
            ...
            <label className="space-y-2 lg:col-span-2"><span className="text-sm font-black text-slate-700 dark:text-slate-200">الفيديو</span><span className="flex min-h-24 cursor-pointer items-center justify-center gap-3 rounded-2xl border border-dashed border-sky-200 bg-sky-50/60 p-4 text-sm font-black text-sky-700 dark:border-sky-500/30 dark:bg-sky-500/10 dark:text-sky-300"><UploadCloud className="h-6 w-6" />{form.file?.name || (editingId ? "اختيار فيديو بديل (اختياري)" : "اختيار ملف MP4")}<input type="file" accept="video/mp4,.mp4" required={!editingId} onChange={(event) => setForm((current) => ({ ...current, file: event.target.files?.[0] ?? null }))} className="sr-only" /></span></label>
