...
export function PlanPaymentModal({
  ...
  return (
    <div
      ...
      dir="rtl"
    >
      <section className="relative grid max-h-[94vh] w-full max-w-5xl overflow-y-auto rounded-[2rem] border border-white/70 bg-white shadow-2xl lg:grid-cols-[0.85fr_1.15fr]">
        <button
          type="button"
          onClick={onClose}
        ...
        <aside className="bg-slate-950 p-6 text-white sm:p-8">
          ...
          <div className="mt-7 space-y-3 border-y border-white/10 py-5">
            {services.slice(0, 4).map((service) => (
              <div key={service} className="flex items-center gap-2 text-sm font-bold text-slate-200">
                <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-emerald-400" />
                <span>{service}</span>
              </div>
            ))}
            {services.length === 0 ? (
              <p className="text-sm font-bold text-slate-300">تشمل خدمات الباقة المحددة.</p>
            ) : null}
          </div>
          ...
          <div className="mt-6 flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-3xl font-black">{total.toLocaleString("ar-SA")} ريال</p>
            </div>
            <ShieldCheck className="h-8 w-8 text-emerald-300" />
          </div>

          <p className="mt-6 text-xs font-bold leading-6 text-slate-400">
            تتم معالجة بيانات البطاقة مباشرة عبر بوابة Moyasar الآمنة.
          </p>
        </aside>
        ...
        <div className="p-6 pt-16 sm:p-8 sm:pt-16">
          <div className="flex items-center gap-3">
            <div className="grid h-11 w-11 place-items-center rounded-2xl bg-emerald-50 text-emerald-700">
              {mode === "online" ? <CreditCard className="h-5 w-5" /> : <Landmark className="h-5 w-5" />}
            </div>
            <div>
              <p className="text-xs font-black text-emerald-700">إتمام الاشتراك</p>
              <h2 id="payment-modal-title" className="mt-1 text-xl font-black text-slate-950">
          ...
          {mode === "online" ? (
            <>
              {isCreatingTransaction ? (
                <div className="mt-8 grid min-h-56 place-items-center rounded-3xl border border-slate-100 bg-slate-50 text-center">
                  <div>
                    <Loader2 className="mx-auto h-7 w-7 animate-spin text-emerald-600" />
                    <p className="mt-3 text-sm font-black text-slate-600">جارٍ تجهيز عملية الدفع...</p>
                  </div>
                </div>
              ...
              <button
                ...
                className="mx-auto mt-5 block text-sm font-black text-slate-500 underline decoration-slate-300 underline-offset-4 transition hover:text-slate-900"
              >
                أفضل التحويل البنكي
              </button>
            </>
            ...
            <>
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                <input
                  ...
                  onChange={(event) => onBankTransferChange({ senderName: event.target.value })}
                ...
                <input
                  ...
                  onChange={(event) => onBankTransferChange({ phone: event.target.value })}
                  placeholder="رقم الجوال"
                  className="h-12 rounded-2xl border border-slate-200 px-4 text-sm font-bold outline-none focus:border-emerald-400"
                />
                <input
                  value={bankTransfer.receiptUrl}
                  onChange={(event) => onBankTransferChange({ receiptUrl: event.target.value })}
                ...
                <textarea
                  ...
                  onChange={(event) => onBankTransferChange({ note: event.target.value })}
              ...
              >
                {isSubmittingBankTransfer ? <Loader2 className="h-5 w-5 animate-spin" /> : <Landmark className="h-5 w-5" />}
                {isSubmittingBankTransfer ? "جارٍ إرسال الطلب..." : "إرسال طلب التحويل"}
              </button>
