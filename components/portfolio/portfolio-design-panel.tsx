...
export function PortfolioDesignPanel({ data, busy, onSave }: {
  ...
  return <section className="space-y-6">
    <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">{PORTFOLIO_THEMES.map((theme) => <PortfolioThemePreviewCard key={theme.id} theme={theme} selected={theme.id === selectedThemeId} disabled={busy} onSelect={() => void selectTheme(theme.id)} />)}</div>
    </div>
