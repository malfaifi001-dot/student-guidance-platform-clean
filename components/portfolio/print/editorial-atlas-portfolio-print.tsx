...
function AtlasCurriculumPages({ output, sectionTitle, physicalDocument }: { output: PortfolioServiceOutput; sectionTitle: string; physicalDocument: PortfolioPhysicalDocument }) {
  ...
  return pageWeeks.map((weeks, index) => (
    <AtlasPage key={`${output.id}-${index}`} sectionLabel={sectionTitle} className="atlas-curriculum-page" indexLabel={index ? "·" : "01"}>
      ...
      <style>{`.atlas-curriculum-page .atlas-curriculum-list{grid-template-columns:repeat(3,minmax(0,1fr));gap:4px}.atlas-curriculum-page .atlas-curriculum-row{display:block;padding:5px}.atlas-curriculum-page .atlas-curriculum-row header{margin-bottom:3px;padding-bottom:3px;border-bottom:1px solid #dce2e6}.atlas-curriculum-page .atlas-curriculum-row header small{white-space:nowrap}.atlas-curriculum-page .atlas-curriculum-row ul{padding-inline-start:11px}`}</style>
...
function AtlasActivityOutputPages({ output, sectionTitle, physicalDocument }: { output: PortfolioServiceOutput; sectionTitle: string; physicalDocument: PortfolioPhysicalDocument }) {
  ...
  return pageChunks.filter((chunk) => chunk.kind !== "curriculum-distribution").map((chunk, index) => (
    <AtlasPage key={`${output.id}-${index}`} pageId={physicalDocument.serviceOutputPages[output.id]?.[index]?.id} outputId={output.id} chunkIndex={index} sectionLabel={sectionTitle} className="atlas-activity-output-page portfolio-service-output-page" indexLabel={String(index + 1).padStart(2, "0")}>
...
function renderValue(value: string | string[], field?: PortfolioReportField, internalLayout?: PortfolioFieldInternalLayout) {
  if (Array.isArray(value)) {
    ...
    return (
      <ul className="atlas-value-list" style={columns > 1 ? { display: "grid", gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))`, gap: ".3mm .7mm" } : undefined}>
  ...
  return (
    <>
      {pages.map((page, pageIndex) => (
        ...
        >
          <header className="atlas-report-title" data-portfolio-page-id={physicalDocument.reportPages[reportId]?.[pageIndex]?.id}>
                  ...
                  >
                    ...
                    <div className="atlas-detail-grid" style={{ gridTemplateColumns: `repeat(${page.fieldColumnCount || 4}, minmax(0, 1fr))` }}>
                      {(page.fieldBands || []).flatMap((band) =>
                        band.items.map(({ field, columnStart, columnSpan: effectiveSpan, row, semanticKind: kind, sourceIndex: index, renderedHeightPx, internalLayout }) => {
    ...
    >
      ...
      {data.portfolio.preferences.showTableOfContents ? (
        <AtlasPage sectionLabel="الفهرس" indexLabel="02">
          ...
          <div className="atlas-index-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))" }}>
            {data.sections
              .filter((section) => section.isEnabled)
      ...
      <style>{`
        .atlas-introduction-page .atlas-heading h2 { font-family: var(--font-cairo), "Cairo", Tahoma, Arial, sans-serif; font-size: 32px !important; font-weight: 900; line-height: 1.3; }
        .atlas-introduction-page .atlas-heading p { font-family: var(--font-cairo), "Cairo", Tahoma, Arial, sans-serif; font-size: 16px !important; font-weight: 600; line-height: 2; max-width: 165mm; text-align: justify; text-justify: inter-word; }
        .atlas-identity-physical-page .portfolio-identity-page-heading h1 { font-family: var(--font-cairo), "Cairo", Tahoma, Arial, sans-serif; font-size: 30px !important; font-weight: 900; line-height: 1.3; }
        .atlas-identity-physical-page .portfolio-educational-identity-content h2 { font-family: var(--font-cairo), "Cairo", Tahoma, Arial, sans-serif; font-size: 17px !important; font-weight: 900; line-height: 1.4; }
        .atlas-identity-physical-page .portfolio-educational-identity-content p { font-family: var(--font-cairo), "Cairo", Tahoma, Arial, sans-serif; font-size: 12px !important; line-height: 1.8; }
        .atlas-identity-physical-page .portfolio-educational-identity-content li { font-family: var(--font-cairo), "Cairo", Tahoma, Arial, sans-serif; font-size: 11px !important; line-height: 1.7; }
      `}</style>
      {sectionEnabled("introduction") ? (
        <div>
          <AtlasPage sectionLabel="المقدمة" indexLabel="03" className="atlas-introduction-page">
            <Heading
              kicker="مدخل الملف"
              title="المقدمة"
              description={
      ...
      {sectionEnabled("educational-identity") ? (
        <AtlasPage sectionLabel="الهوية التعليمية" indexLabel="04" className="portfolio-identity-physical-page atlas-identity-physical-page">
