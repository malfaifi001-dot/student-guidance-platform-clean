...
export function PublicReportSignatureForm({
  ...
  async function confirm() {
    ...
    try {
      setSaving(true);
      setError("");
      const response = await fetch(`/api/report-signature/${encodeURIComponent(token)}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ dataUrl: signature, consentToReuse }),
      });
    ...
    <main className="min-h-screen bg-slate-100 px-3 py-6" dir="rtl">
      <div className="mx-auto max-w-7xl space-y-5">
        <header className="rounded-[2rem] border border-blue-100 bg-white p-6 shadow-sm">
          <p className="text-sm font-black text-blue-700">طلب توقيع تقرير</p>
          <h1 className="mt-2 text-2xl font-black text-slate-950">مراجعة التقرير وتوقيعه</h1>
          <p className="mt-3 text-sm font-bold leading-7 text-slate-600">
            فضلاً راجع التقرير التالي كاملًا قبل إضافة توقيع {principalName || "مدير المدرسة"}.
          </p>
        </header>
        ...
        {!showSignature ? (
          <div className="flex justify-center">
            <button type="button" onClick={() => setShowSignature(true)} className="rounded-2xl bg-blue-700 px-7 py-3 text-sm font-black text-white hover:bg-blue-800">
              إضافة التوقيع
            </button>
          </div>
        ) : (
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          <h2 className="text-xl font-black text-slate-950">إضافة توقيع المدير</h2>
          ...
          {signature ? (
            <>
              ...
              <p className="mt-2 text-xs font-bold leading-6 text-slate-500">ترك المربع دون تحديد يعني استخدام التوقيع لهذا التقرير فقط.</p>
            </>
          ) : null}
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            ...
            <button type="button" onClick={() => void confirm()} disabled={!signature || saving} className="rounded-2xl bg-blue-700 px-6 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-50">
              {saving ? "جاري حفظ التوقيع..." : "تأكيد توقيع التقرير"}
            </button>
          </div>
        </section>
        )}
...
function StatusCard({ title, text, tone }: { title: string; text: string; tone: "green" | "red" }) {
  return (
    <main className="grid min-h-screen place-items-center bg-slate-100 p-4" dir="rtl">
      <section className={`w-full max-w-xl rounded-[2rem] border bg-white p-8 text-center shadow-sm ${tone === "green" ? "border-emerald-200" : "border-red-200"}`}>
        <h1 className={`text-2xl font-black ${tone === "green" ? "text-emerald-700" : "text-red-700"}`}>{title}</h1>
