...
export function AdminSalesExperiencePanel() {
  ...
  async function load() {
    setLoading(true);
    try {
      const response = await fetch(`/api/dashboard/admin/sales-experience?q=${encodeURIComponent(query)}`, { cache: "no-store" });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "تعذر تحميل وضع تجربة البيع.");
      setGlobalMode(payload.globalMode === "BAG" ? "BAG" : "SERVICE");
      setUsers(Array.isArray(payload.users) ? payload.users : []);
    } catch (error) {
      setFeedback(error instanceof Error ? error.message : "تعذر تحميل وضع تجربة البيع.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <section dir="rtl" className="space-y-4 rounded-[1.75rem] border border-violet-100 bg-gradient-to-br from-violet-50 via-white to-sky-50 p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <button type="button" onClick={() => void load()} className="rounded-xl bg-white p-2 text-slate-600 shadow-sm" aria-label="تحديث"><RefreshCw className="h-4 w-4" /></button>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4">
        <h3 className="font-black text-slate-950">مستخدمو تجربة وضع الحقيبة</h3>
        <div className="mt-3 flex gap-2">
          <div className="relative flex-1"><Search className="absolute right-3 top-3 h-4 w-4 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void load(); }} placeholder="ابحث بالاسم أو البريد الإلكتروني" className="w-full rounded-xl border border-slate-200 py-2.5 pl-3 pr-9 text-sm font-bold outline-none focus:border-violet-400" /></div>
