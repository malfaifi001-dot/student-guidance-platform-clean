              ...
              {columns.map((column, columnIndex) => (
                ...
                >
                  {column.map((module) => {
                    ...
                    const card = (
                      <article className="flex min-h-[4.4rem] items-center justify-center gap-2 rounded-2xl border border-slate-100 bg-slate-50 px-3 py-2.5 text-center shadow-sm transition active:scale-[0.99]">
                        <Icon className="h-4 w-4 shrink-0 text-sky-700" aria-hidden="true" />
                        <h3 className="min-w-0 text-sm font-black leading-5 text-slate-950">
                          {module.title}
                        </h3>
                      </article>
                    );
