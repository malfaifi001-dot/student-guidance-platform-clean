export type BatchCertificateRenderRecord = {
  id: string;
...
function renderCertificatePage(certificate: BatchCertificateRenderRecord) {
  const data = readJsonObject(certificate.dataJson);
...
export function renderCertificatesBatchDocumentHtml(certificates: BatchCertificateRenderRecord[]) {
  return `<!doctype html>
...
<body>
  ${certificates.map(renderCertificatePage).join("\n")}
</body>
</html>`;
}