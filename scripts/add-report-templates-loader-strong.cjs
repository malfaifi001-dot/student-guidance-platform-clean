...
const pattern =
  /  const activeTemplate = useMemo\(\(\) => \{\r?\n    return templates\.find\(\(template\) => template\.id === activeTemplateId\);\r?\n  \}, \[templates, activeTemplateId\]\);\r?\n/;

const match = content.match(pattern);

if (!match) {
  throw new Error("لم أجد activeTemplate useMemo لإضافة Loader بعده.");
}
...
content = content.replace(pattern, loader);

fs.writeFileSync(path, content, "utf8");

console.log("تمت إضافة Loader القوالب المحفوظة بنجاح.");
