...
function replaceOnceByRegex(content, regex, replacement, label) {
  if (!regex.test(content)) {
    throw new Error(`لم أجد مكان الاستبدال: ${label}`);
  }
...
if (!preview.includes("const builderTemplate =")) {
  preview = replaceOnceByRegex(
    preview,
    /(\s{2}const officialReport = buildOfficialReportData\(\{\s*report,\s*reportValues,\s*parsedEditableContent,\s*evidenceLayout: selectedEvidenceLayout,\s*viewMode: selectedViewMode,\s*\}\);\s*)return \(/,
    `$1const builderTemplate =
  ...
  return (`,
    "preview insert builderTemplate"
  );
}

preview = replaceOnceByRegex(
  preview,
  /<section className=\{studioMode \? "mx-auto" : "mx-auto max-w-\[260mm\]"\}>\s*<ReportDocumentRenderer\s*identity=\{identity\}\s*report=\{officialReport\}\s*templateId=\{selectedTemplate\}\s*showCover=\{showCover\}\s*evidenceLayout=\{selectedEvidenceLayout\}\s*\/>\s*<\/section>/,
...
if (!studio.includes("function getBuilderTemplateFromSnapshot")) {
  studio = replaceOnceByRegex(
    studio,
    /(type ReportFieldLookupItem = \{[\s\S]*?\};\s*)export default async function ReportStudioPage/,
    `$1
...
if (!studio.includes("const builderTemplate =")) {
  studio = replaceOnceByRegex(
    studio,
    /(\s{2}const liveCaseValues = buildStudioReportValues\(\s*normalizedCaseValues,\s*workflowValueOverrides\s*\);\s*)const normalizedReport = \{/,
    `$1const builderTemplate =
    getBuilderTemplateFromSnapshot(report.templateSnapshot) ||
  ...
  const normalizedReport = {`,
    "studio insert builder conditional"
  );
}

fs.writeFileSync(studioPath, studio, "utf8");

console.log("تم التعديل فعليًا بنجاح.");
