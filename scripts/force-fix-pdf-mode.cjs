...
function mustMatch(content, regex, label) {
  if (!regex.test(content)) {
    throw new Error(`لم أجد الموضع المطلوب: ${label}`);
  }
}
...
/* 1) Add pdfMode prop */
live = live.replace(
  /type ReportTemplateLivePreviewProps = \{\s*template: ReportTemplateBuilderModel;\s*snippets: ReportTextSnippet\[\];\s*previewCaseData: RuntimePreviewCaseData \| null;\s*\};/,
...
/* 2) Replace the whole exported component only, keep PreviewBlock and below */
const componentRegex =
  /export function ReportTemplateLivePreview\([\s\S]*?\n\}\n\nfunction PreviewBlock/;

mustMatch(live, componentRegex, "ReportTemplateLivePreview component");
        ...
        {template.pages.map((page, pageIndex) => (
          ...
          >
            ...
            >
              ...
              {page.kind === "cover" ? <CoverPreviewFooter template={template} /> : null}
            </div>
          </article>
        ))}
      </div>
...
console.log("تم إدخال pdfMode فعليًا في LivePreview و Preview page.");
